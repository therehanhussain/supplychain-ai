...
const IndustryGraph: React.FC<IndustryGraphProps> = ({ expId: propExpId }) => {
  ...
  useEffect(() => {
    ...
    const handleTimelineChange = (event: any) => {
      const { step } = event.detail;
      console.log('IndustryGraph: Timeline changed to step:', step);
      setCurrentStep(step);

      // 如果开启了行为高亮，触发高亮更新
      if (behaviorHighlightEnabled && (edgeDisplayMode === 'transaction' || edgeDisplayMode === 'communication')) {
        setTimeout(() => {
          highlightBehaviorConnectionsByStep(step);
        }, 100);
      }
    ...
    window.addEventListener('timelineChange', handleTimelineChange as EventListener);

    return () => {
      window.removeEventListener('filterByStep', handleFilterByStep as EventListener);
      window.removeEventListener('timelineChange', handleTimelineChange as EventListener);
    };
