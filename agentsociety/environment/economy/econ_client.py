...
class EconomyClient:
    ...
    async def get(
        ...
        if id == -1:
            if isinstance(key, list):
                return [self.global_state.get(k) for k in key]
            else:
    ...
    async def update(
        ...
        if id == -1:
            if mode == "merge":
                # 取现有列表
                existing_list = self.global_state.get(key, [])
                if not isinstance(existing_list, list):
                    raise ValueError(f"Expected a list for {key}, but got {type(existing_list)}")

                if not isinstance(value, list):
                    raise ValueError(f"Value must be a list to merge for {key}")

                 # 按 company_id 更新/覆盖
                existing_dict = {item["company_id"]: item for item in existing_list}
                for v in value:
                    cid = v.get("company_id")
                    if cid is None:
                        raise ValueError("Each value must contain 'company_id'")
                    # 覆盖或新增
                    existing_dict[cid] = v
                # 更新 global_state
                self.global_state[key] = list(existing_dict.values())
            else:
                # 默认替换
