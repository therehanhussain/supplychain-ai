...
class LLM:
    ...
    def get_client_by_index(self, index: int):
        # 选取指定LLM 
        if index < 0 or index >= min(len(self._aclients), len(self.configs)):
            raise ValueError(
                f"Invalid client index: {index} vs {len(self._aclients)} & {len(self.configs)}"
            )
        config = self.configs[index]
        client = self._aclients[index]
        return config, client
    ...
    async def atext_request(
        ...
        for attempt in range(retries):
            self._total_calls += 1
            config, (client, semaphore) = self.get_client_by_index(intelligence_level-1)
            response = None
