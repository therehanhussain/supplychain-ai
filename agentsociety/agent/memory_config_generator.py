...
class MemoryConfigGenerator:
    ...
    def generate(self, i: int, agent_id: int = None):
        """
        Generate memory configuration.

        Args:
            i (int): The index of the memory configuration to generate. Used to find the i-th memory configuration in the file.
            agent_id (int, optional): The agent ID to pass to the memory config function.

        Returns:
            tuple[dict[str, MemoryT], dict[str, MemoryT], dict[str, Any]]: The memory configuration.
        """
        # 检查memory_config_func是否支持agent_id参数
        import inspect
        sig = inspect.signature(self._memory_config_func)
        if 'agent_id' in sig.parameters and agent_id is not None:
            extra_attrs, profile, base = self._memory_config_func(self._distributions, agent_id=agent_id)
        else:
