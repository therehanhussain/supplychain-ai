...
class EnterpriseAgentBase(Agent):
    def __init__(
        self,
        id: int,
        name: str,
    ...
    async def _bind_to_economy(self):
        ...
        products = await self.status.get("products")
        product_stocks = await self.status.get("product_stocks")
        available_materials = await self.status.get("available_materials")
        company_capacity = await self.status.get("company_capacity")
        fund = await self.status.get("fund")

        price = await self.status.get("price")
        inventory = await self.status.get("inventory")
        employees = await self.status.get("employees")
        demand = await self.status.get("demand")
        currency = await self.status.get("currency")
        sales = await self.status.get("sales")
        # await self.environment.economy_client.add_agents()
...
class NewFirmAgentBase(EnterpriseAgentBase):
    """
    Represents a newfirm agent within the simulation environment.
    """
