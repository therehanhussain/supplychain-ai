...
class Agent(ABC):
    ...
    async def send_message_to_agent(
        self, to_agent_id: int, content: str, type: str = "social", topic: str = "none"
    ):
        """
        ...
        day, t = self.environment.get_datetime()
        # send message with `Messager`
        if type not in ["social", "economy", "operation", "operation-price", "operation-price-get", "operation-deal","operation-build","operation-reject","update_sell_price","update_payment_method","check_manufacturing_decison"]:
