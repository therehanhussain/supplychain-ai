...
class AgentSociety:
    ...
    async def init(self):
        ...
        # ====================
        get_logger().info(f"Initializing environment111...")
        ...
        for agent_config in self._config.agents.firms:
            if agent_config.memory_distributions is None:
                agent_config.memory_distributions = {}
            assert (
                "hq_aoi_id" not in agent_config.memory_distributions
            ), "hq_aoi_id is not allowed to be set in memory_distributions because it will be generated in the initialization"
            agent_config.memory_distributions["hq_aoi_id"] = DistributionConfig(
    ...
    async def step(self, num_environment_ticks: int = 1) -> Logs:
        ...
        try:
            ...
            get_logger().info(
                f"Start simulation day {day} at {t} tick {tick}, step {self._total_steps}"
            )
            tasks = []
            for group in self._groups.values():
        ...
        try:
            for step in self.config.exp.workflow:
                ...
                if step.type == WorkflowType.STEP:
                    ...
                    get_logger().info(f"days: {days}")
                    get_logger().info(f"remain: {remain}")
                    for _ in range(days):
