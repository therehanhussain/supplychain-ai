...
class AgentGroup:
    ...
    async def init(self):
        ...
        for agent_init in self._agent_inits:
            ...
            memory_dict = memory_config_generator.generate(index_for_generator, agent_id=id)  # 传递agent_id
    ...
    async def save_status(self, day: int, t: int):
        ...
        policy_survey_list = []
        for agent in self._agents:
            if isinstance(agent,NewFirmAgentBase):
                firmsId.append(agent.id)
                has_join = await agent.status.get("has_join")
                name = await agent.status.get("name")
                policy_survey_list.append({"company_id":agent.id,"company_name":name,"has_join":has_join})
        for agent in self._agents:
            if isinstance(agent, CitizenAgentBase):
                position = await agent.status.get("position")
                # get_logger().info("positionposition:{position}")
            ...
            elif isinstance(agent,NewFirmAgentBase):
                position = await agent.status.get("position")
                x = position["xy_position"]["x"]
                y = position["xy_position"]["y"]
                lng, lat = self.environment.projector(x, y, inverse=True)
                if "aoi_position" in position:
                    ...
                    parent_id = None
                fund = await agent.status.get("fund")
                product_inventory = await agent.status.get("product_inventory")
                inventory_system = await agent.status.get("inventory_system")  # 添加这行
                current_plan = await agent.status.get("current_plan")
                action = ""
                firm_list = firmsId
                await agent.status.update("firm_list",firm_list)
                # if current_plan is not None and current_plan:
                #     intention = current_plan.get("target", "Other")
                #     step_index = current_plan.get("index", 0)
                #     action = current_plan.get("steps", [])[step_index].get(
                    ...
                    status=jsonc.dumps(
                        {
                            ...
                            "inventory_system": inventory_system,  # 添加这行
                        },
                        ensure_ascii=False,
                    ),
                    created_at=created_at,
                ...
                await agent.status.update("policy_survey", policy_survey_list)
                status = StorageStatus(
                    id=agent.id,
                    day=day,
                    t=t,
                    ...
                    status=jsonc.dumps(
                        {
                            ...
                            "employees": employees,
                            "policy_survey": policy_survey_list,
                        },
                        ensure_ascii=False,
                    ),
                    created_at=created_at,
